...
-- Uses ON CONFLICT to handle race conditions and existing profiles
...
EXCEPTION
  WHEN OTHERS THEN
    -- Log error but don't fail the trigger (allow user creation to succeed)
...
-- ================================================
-- SECTION 14: GET MY ENROLLMENTS FUNCTION (For Students)
-- ================================================
-- This function allows students to fetch their own enrollments with class and instructor details
-- Bypasses RLS complexity for reliable data loading
...
BEGIN
  ...
  IF v_caller_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'not_authenticated', 'enrollments', '[]'::jsonb);
  END IF;
  
  -- Get enrollments with class and instructor details
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', e.id,
      'class_id', e.class_id,
...
-- ================================================
-- SECTION 17: STORAGE BUCKET POLICIES FOR PROCTORING EVIDENCE
-- ================================================
-- This creates the proctoring-evidence bucket and policies
-- Allows students to upload evidence screenshots during exams
